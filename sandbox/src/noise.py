...
from .utilities import import_gmat_file, import_wmat_file, interpolate
...
class InterpolatedNoiseModel(BaseNoiseModel):
    ...
    def apply_programming_noise_to_conductance(self, g_target: torch.Tensor, neg: bool) -> torch.Tensor:
        ...
        if neg:
            ...
            g_prog = torch.clamp(g_prog, max = 0)
        else:
...
class SingleConductanceNoiseModel(BaseNoiseModel):
    """Single conductance (no differential compensation) noise model. """

    def __init__(self, file_path: str, type:str, debug:bool= False, levels : int = None, force_interpolation: bool = False, compensation: bool = False ,**kwargs):
        super().__init__(**kwargs)
        self.chosen_type = type
        variables = interpolate(levels = levels, type = type, file_path = file_path, force_interpolation = force_interpolation, compensation=compensation, conductance_type="g")
        types = variables['str']
        ww_mdn = variables['gg_mdn']
        ww_std = variables['gg_std']
...
class CompensatedSingleConductanceNoiseModel(BaseNoiseModel):
    """Single conductance (no differential compensation) noise model. """
    ...
    def apply_programming_noise(self, weights: Tensor) -> Tuple[Tensor, List[Tensor]]:
        ...
        # compute the sum of all the condunctances inclusive of all the tiles
        sum_currents_ref = 0
        sum_currents_drift = 0

        for i,g_target in enumerate(target_conductances):
            sum_currents_ref += torch.sum(g_target)
            if i % 2 == 0:
                neg = False
            else:
                neg = True
            noisy_conductances.append(self.apply_programming_noise_to_conductance(g_target, neg))
            nu_drift_list.append(self.generate_drift_coefficients(g_target))
            sum_currents_drift += torch.sum(noisy_conductances[i])
        
        alpha = sum_currents_ref / sum_currents_drift
        # print("Alpha value: ", alpha)
        for i in range(len(noisy_conductances)):
            noisy_conductances[i] = noisy_conductances[i] * alpha
        noisy_weights = self.g_converter.convert_back_to_weights(noisy_conductances, params)
...
class ExperimentalNoiseModel(BaseNoiseModel):
    """Experimental noise (differential weight encoding) model. """

    def __init__(self, file_path: str, type:str, debug:bool= True, levels : int = None, force_interpolation: bool = False, compensation: bool = False ,**kwargs):
        super().__init__(**kwargs)
        self.chosen_type = type
...
class CompensatedExperimentalNoiseModel(BaseNoiseModel):
    """Experimental noise (differential weight encoding) model. """
